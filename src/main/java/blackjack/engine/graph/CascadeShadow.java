...
/*
 * Responsible of calculating the matrices required to render the shadow maps
 * from light perspective. Stores the projection view matrix (from light perspective)
 * for a specific cascade shadow split and the far plane distance for its
 * ortho-projection matrix (splitDistance)
 */
public class CascadeShadow {
    ...
    // initialize a list of cascade shadows instances with the proper values
    public static void updateCascadeShadows(List<CascadeShadow> cascadeShadows, Scene scene) {
        ...
        float[] cascadeSplits = new float[Consts.SHADOW_MAP_CASCADE_COUNT];
        ...
        for (int i = 0; i < Consts.SHADOW_MAP_CASCADE_COUNT; i++) {
            float p = (i + 1) / (float) (Consts.SHADOW_MAP_CASCADE_COUNT);
        ...
        for (int i = 0; i < Consts.SHADOW_MAP_CASCADE_COUNT; i++) {
            ...
            //calc the coords of th center and radius of the current split
            ...
            /* 
             * calculate the view matrix from the light POV and the orthographic 
             * projMatrix as well as the split distance (in cam view coordinates)
            */
            ...
            Matrix4f lightOrthoMatrix = new Matrix4f().ortho(
                ...
                minExtents.y, maxExtents.y, 0.0f,
                maxExtents.z - minExtents.z, true
            );
